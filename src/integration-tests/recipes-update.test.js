...
describe('Testa update de recipes', () => {
  const users = [
  ...
  const recipe = {
    ...
    name: 'Receita de frango do admin',
    ingredients: 'Frango',
  ...
  describe('Quando não está autenticado', () => {
    ...
    before(async () => {
      ...
      response = await chai.request(server).put(`/recipes/${CHICKEN_ID}`);
    });
  ...
  describe('Quando não está autorizado', () => {
    ...
    before(async () => {
      ...
      response = await chai.request(server).put(`/recipes/${CHICKEN_ID}`)
        .send({ name: 'Receita de frango do Jacquin', }).set('authorization', token);
    ...
    it('Não atualiza receita no banco', async () => {
  ...
  describe('Quando está autorizado', () => {
    ...
    it('Atualiza receita no banco', async () => {
